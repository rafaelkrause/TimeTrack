...
Referência completa das opções configuráveis do TimeTrack.
...
| Windows (instalador) | `%APPDATA%\TimeTrack\config.json` |
...
- A UI também persiste a escolha corrente em `localStorage` (chave `tt-theme`).
