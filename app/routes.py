...
def pause_activity():
    ...
    current = storage.get_current_activity()
    if not current:
        return jsonify({"error": "Nenhuma atividade ativa"}), 400
    try:
        current.pause()
        storage.save_activity(current)
        _bump_revision()
...
def resume_activity():
    ...
    current = storage.get_current_activity()
    if not current:
        return jsonify({"error": "Nenhuma atividade para retomar"}), 400
    try:
        current.resume()
        storage.save_activity(current)
        _bump_revision()
...
def update_activity(activity_id):
    ...
    if "start_time" in data:
        if not _validate_time(data["start_time"]):
            return jsonify({"error": "Horário de início inválido (HH:MM)"}), 400
    ...
    if "end_time" in data and activity.ended_at:
        if not _validate_time(data["end_time"]):
            return jsonify({"error": "Horário de fim inválido (HH:MM)"}), 400
...
_PHRASES_PATH = Path(__file__).parent / "data" / "phrases.json"
_phrases_cache = None
...
def get_phrase(category):
    ...
    phrases = _load_phrases().get(category, [])
    if not phrases:
        return jsonify({"phrase": None})
...
def export():
    ...
    if not from_str or not to_str:
        return jsonify({"error": "Parâmetros 'from' e 'to' são obrigatórios"}), 400
    ...
    if (to_date - from_date).days > 366:
        return jsonify({"error": "Período máximo de exportação: 1 ano"}), 400
    if fmt not in ("csv", "tsv"):
        return jsonify({"error": "Formato inválido (csv ou tsv)"}), 400
    ...
    for key in data:
        ...
        if key == "user_name":
            ...
            try:
                ...
                return jsonify({"error": "Meta de apontamento inválida"}), 400
        elif key == "port":
            try:
                val = int(val)
                if not (1024 <= val <= 65535):
                    return jsonify({"error": "Porta inválida (1024-65535)"}), 400
            except (ValueError, TypeError):
...
def update_shifts():
    ...
    for day in _VALID_DAYS:
        ...
        if not isinstance(shifts, list) or len(shifts) > MAX_SHIFTS_PER_DAY:
            return jsonify({"error": f"Turnos inválidos para {day}"}), 400
        day_shifts = []
        for s in shifts:
            if not isinstance(s, dict):
                return jsonify({"error": f"Turno inválido em {day}"}), 400
            start = s.get("start", "")
            end = s.get("end", "")
            if not _validate_time(start) or not _validate_time(end):
                return jsonify({"error": f"Horário inválido em {day}"}), 400
